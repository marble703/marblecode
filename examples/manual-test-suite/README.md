...
The deterministic suite currently covers 84 cases across these groups:
...
- planner execution now covers both conservative wave scheduling and conflict-policy behavior before real concurrent subagent work is trusted by default
...
- planner runtime tests cover execution-state metadata, persisted recovery snapshot helpers, active-wave resume behavior, guarded owner reuse, unrelated writer drop, interrupted/completed planning-window resume boundaries, fallback-path resume inside partial windows, and lock preservation across resume
