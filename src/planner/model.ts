...
function buildPlannerSystemPrompt(maxSteps: number, planningWindowWaves: number, executeSubtasks: boolean): string {
  return [
    ...
    'You may also include optional step fields such as maxAttempts, fallbackStepIds, dependsOnFiles, producesFiles, ownershipTransfers, conflictsWith, conflictDomains, failureTolerance, and dependencyTolerances when they help execution planning.',
    ...
    'Use dependencyTolerances only when a downstream non-verify step can continue after a degraded dependency failure; keys must reference declared dependencies and verify steps should normally keep required dependencies.',
