...
2. 规划器可使用只读工具进行搜索。
3. 规划器返回结构化的 `plan`、`plan_update`、`tool_call` 或 `final` 响应。
...
- 针对失败步骤的重试、备用模型选择、graph fallback 激活以及经过 proposal 校验且受 bounded scope 约束的本地重新规划
