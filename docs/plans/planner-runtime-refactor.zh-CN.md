...
当前状态：阶段 1 已完成。runtime primitives 已落地，`execute.ts` 的 turn-level 判定已收口到 runtime runner：ready/pending/batch 选择、complete、no-ready blocked、runtime-lock blocked 都已通过 runner decision 显式建模；同时 blocked/completion/planning-window completion 以及 skip/verify feedback/wave feedback 的 outcome 数据构造也已下沉到 runner helper。batch selection 默认走 ready queue，并在 fallback-ready 场景保留 legacy wave defer。verify/wave/fallback/replan/resume 的实际执行体仍保留在兼容层，这也是阶段 2 开始前的明确边界。
...
- 阶段 2：收敛 failure handling，弱化 graph fallback 主路径
