...
- `src/planner/model.ts`、`parse.ts`、`artifacts.ts`、`prompts.ts`、`state.ts`、`recovery.ts`、`utils.ts`、`execute-wave.ts`、`execute-verify.ts`：已拆出的 planner 辅助模块，分别处理请求构造、解析、artifact、提示词、状态刷新、恢复流程、执行波次逻辑、verify 步骤执行和共享工具逻辑
