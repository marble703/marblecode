...
- Planner execute also builds `execution.graph.json`, `execution.locks.json`, and `execution.state.json`; local replan now also writes `replan.proposal.<stepId>.json` and `replan.rejected.<stepId>.json`, so future concurrency and recovery work can rely on explicit waves, conflicts, file ownership, persisted execution snapshots, and auditable replan artifacts rather than implicit step order.
