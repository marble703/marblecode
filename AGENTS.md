...
- Planner execute now writes `execution.feedback.json` per wave epoch, tracking changed files, undeclared changed files, verify failures, and step summaries. Undeclared changed files trigger replan feedback events, and affected subgraph selection uses `buildPlannerAffectedSubgraph()`. Replan requests now include execution feedback, and append validation checks active lock/wave conflicts via `validateAppendActiveWaveConflict()`.
