...
- `src/agent`: JSON-step agent loop and apply-failure messaging. Request/system-prompt construction now lives in `src/agent/model.ts`, model-step parsing in `src/agent/parse.ts`, runtime orchestration in `src/agent/runtime.ts`, user-facing failure/help text in `src/agent/messages.ts`, and `src/agent/index.ts` remains the public entrypoint plus rollback helper.
- `src/config`: config schema defaults and project/local config loading.
- `src/planner`: `src/planner/index.ts` is the public `runPlanner()` entrypoint; `src/planner/loop.ts` owns the planner runtime loop, execute orchestration lives under `execute*.ts` modules, execution-state/strategy helpers live in `execution-*.ts` modules, `src/planner/runtime.ts` owns planner request/state/result helpers, and `src/planner/execution-machine.ts` now centralizes execution phase transitions and dispatch semantics.
- `src/planner/model.ts`, `src/planner/parse.ts`, `src/planner/artifacts.ts`, `src/planner/prompts.ts`, `src/planner/state.ts`, `src/planner/recovery.ts`, `src/planner/replan-merge.ts`, `src/planner/read-api.ts`, `src/planner/view-model.ts`, `src/planner/ownership.ts`, `src/planner/utils.ts`: planner helper modules split out of the old monolithic planner entrypoint. `view-model.ts` is the internal read-model aggregation layer, while `read-api.ts` is the narrower read-only facade for planner session list/detail consumers.
...
- `src/tools`: built-in tools. `search_text` now supports regex flags plus line/column/context match locations. Provider-compatible registry internals live under `src/tools/provider.ts`, `src/tools/registry.ts`, `src/tools/types.ts`, `src/tools/setup.ts`, and `src/tools/logging.ts`.
- `src/tools`: built-in tools. Planner-facing tools now also include read-only git helpers such as `git_status`, `git_log`, `git_show`, and `git_diff_base`. Local readonly artifact-backed sources now live in `src/tools/local-artifacts.ts`, `src/tools/local-diagnostics-provider.ts`, `src/tools/local-symbols-provider.ts`, and `src/tools/local-references-provider.ts`; `src/tools/diagnostics-provider.ts` remains a deterministic fixture provider for regression coverage.
- `src/tui`: interactive terminal UI plus planner session rendering helpers. Shared TUI types now live in `src/tui/types.ts`, slash-command parsing in `src/tui/commands.ts`, paste/patch-confirm helpers in `src/tui/paste.ts`, state refresh in `src/tui/state.ts`, screen rendering in `src/tui/render.ts`, recent-session projection in `src/tui/recent-sessions.ts`, planner session actions in `src/tui/session-actions.ts`, prompt dispatch in `src/tui/run-prompt.ts`, and live planner inspection in `src/tui/planner-live.ts`; `src/tui/agent-repl.ts` now only owns the top-level loop.
...
- `src/session`: session creation, persistence, cleanup, rollback session resolution, and storage-scoped recent-session entry listing. Planner-rich recent-session projection belongs in `src/tui/recent-sessions.ts`, not in the session storage layer.
