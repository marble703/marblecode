...
- Planner execute now uses those wave/conflict/lock artifacts directly; with `maxConcurrentSubtasks > 1`, same-wave write steps may run concurrently when their file scopes and conflict domains do not overlap, and fallback-completed nodes can now satisfy downstream dependencies of failed source steps.
